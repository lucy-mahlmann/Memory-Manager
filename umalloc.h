...
// variable. This is the value that represents the size of the block minues the metadata that holds the header (which is 16bits). 
size_t get_size(memory_block_t *block);
// Checks if the block is not NULL and then dereferences the block pointer to access the next field
// and returns the memory_block_t pointer to the next block in the free list or NULL if it is the last free block in the free list.
...
// Find the first block that will fit the size that the user has requested in umalloc, if none is found then call extend to extend the 
// heap by size. Then if necessary remove the found block from free list, split the found block, and return the block.
memory_block_t *find(size_t size);
// Use csbrk to get a block of memory of size and immediately return that block to be given to the user in umalloc.
